...
class Critic:
    ...
    def evaluate_tool(self, python_file):
        ...
        query = "Does this code work as intended. Answer with only yes or no. (one word): {}".format(
            read_txt_file_as_string(python_file)
        )
    ...
    def evaluate_task(self, web_voyager, task):
        ...
        critc_prompt = read_txt_file_as_string("prompts/critic.txt").format(
            task, str(prev), str(curr)
        )
        response = query_claude(critc_prompt)
        return response
